...
object FlickrLoader extends DataLoader {
  ...
  def main(args: Array[String]) {
    ...
    val parser = new OptionParser("Load flickr resource") {
      ...
      opt("mlm", "modifiedLastMonth", "Harvest the last month of records", { modifiedLastMonth = true })
      opt("mld", "modifiedLastDay", "Harvest the last day of records", { modifiedLastDay = true })
